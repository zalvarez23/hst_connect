...
angular.module("app.controllers").controller("messagesCtrl-option", [
  "$rootScope",
  "$scope",
  "$location",
  "$ionicPopup",
  ...
  function (
    ...
    $scope.showMessageByClaims = function () {
      $scope.typeMessage = 1;
      $scope.showModalMessage();
    };
    ...
    // validate if coming from claims view

    setTimeout(() => {
      const claimId = $stateParams.claimId;
      if (!claimId) return;
      ($scope.params.textMessage = `Balance Bill Request for HST ID ${claimId}`),
        $scope.showMessageByClaims();
    }, 500);
    // ----------------------------------
    // HELPER FUNCTIONS
    // ----------------------------------
    ...
    // PRUEBA DE AGREGAR
    $ionicModal
      .fromTemplateUrl("my-modal-message.html", {
        scope: $scope,
    ...
    $ionicModal
      .fromTemplateUrl("my-modal-attachment.html", {
        scope: $scope,
        animation: "slide-in-up",
      })
      .then(function (modal) {
    ...
    $scope.addUserMessage = function () {
      ...
      $ionicLoading.show({
        template:
          "<p>" +
          $translate.instant("LOADING_SEND_MESSAGE") +
      ...
      if ($scope.typeMessage == 1) {
        ...
        addMessagesServicesBills
          ...
          .$promise.then(addUserMessageResponse, addUserMessageError);
      } else {
    ...
    function getUserMessages() {
      ...
      messages
        ...
        .$promise.then(getUserMessagesResponse, getUserMessagesError);
    }
    ...
    function GetDraftAttachments() {
      ...
      getDraftAttachments
        ...
        .$promise.then(GetDraftAttachmentsResponse, GetDraftAttachmentsError);
    }
    ...
    var deleteDraftAttachment = function (Id) {
      ...
      $ionicLoading.show({
        template:
          "<p>" +
          $translate.instant("LOADING_DELETE_FILE") +
          "</p><ion-spinner></ion-spinner>",
      });
      ...
      deleteAttachment
        ...
        .$promise.then(DeleteAttachmentsResponse, DeleteAttachmentsError);
    };
    ...
    $scope.uploadPhoto = function () {
      ...
      if ($scope.typeMessage == 1) {
        var server =
          apiUrl.baseUrl +
          "/UploadBalanceBill?MemberId=" +
          localStorageFactory.data.MemberId +
          "&GroupId=" +
          localStorageFactory.data.GroupId +
          "&DeviceId=" +
          localStorageFactory.getDeviceID() +
          "&UserId=" +
          localStorage.getItem("User_Id");
      } else {
        var server =
          apiUrl.baseUrl +
          "/UploadUserMessageAttachment?MemberId=" +
          localStorageFactory.data.MemberId +
          "&GroupId=" +
          localStorageFactory.data.GroupId +
          "&DeviceId=" +
      ...
      $cordovaFileTransfer.upload(server, $scope.uriImg, optionsU).then(
        function (response) {
          $ionicLoading.hide();
          var alertPopup = $ionicPopup.alert({
            title: "Success",
